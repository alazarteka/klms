...
#[test]
fn spec_grammar_matches_the_command_contract() {
    let output = binary().arg("spec").output().unwrap();
    assert!(output.status.success());
    let grammar = String::from_utf8(output.stdout).unwrap();
    let contract = fs::read_to_string(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/docs/COMMAND_CONTRACT.md"
    ))
    .unwrap();
    let marker = "<!-- generated by `klms spec` -->\n```text\n";
    let start = contract
        .find(marker)
        .expect("contract has generated grammar")
        + marker.len();
    let end = contract[start..].find("```").unwrap() + start;
    assert_eq!(
        contract[start..end].trim_end(),
        grammar.trim_end(),
        "docs/COMMAND_CONTRACT.md is stale; paste the output of `klms spec`"
    );
    let json: Value =
